...
const MAIL_FROM = process.env.MAIL_FROM || 'Connectik <onboarding@resend.dev>';

function parseFrom(str) {
  const m = str.match(/^\s*(.+?)\s*<\s*(.+?)\s*>\s*$/);
  if (m) return { name: m[1], email: m[2] };
  return { name: 'Connectik', email: str.trim() };
}
...
function verifyEmailTemplate({ name, link }) {
  ...
  return {
    subject: 'Verify your Connectik email',
    html: `
      <div style="font-family:Segoe UI,Arial,sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#222">
        <h2 style="color:#0a66c2;margin:0 0 16px">Welcome to Connectik, ${safeName}!</h2>
        ...
        <p style="font-size:12px;color:#888;margin-top:32px">If you didn't sign up for Connectik, you can ignore this email.</p>
      </div>`,
    text: `Hi ${safeName},\n\nPlease verify your Connectik email by visiting:\n${link}\n\nIf you didn't sign up, ignore this email.`,
  };
}
...
function resetOtpTemplate({ name, otp }) {
  const safeName = (name || 'there').replace(/[<>]/g, '');
  return {
    subject: 'Your Connectik password reset code',
    html: `
      <div style="font-family:Segoe UI,Arial,sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#222">
        <h2 style="color:#0a66c2;margin:0 0 16px">Password reset code</h2>
        <p>Hi ${safeName}, use the code below to reset your Connectik password. It expires in 10 minutes.</p>
    ...
    text: `Hi ${safeName},\n\nYour Connectik password reset code is: ${otp}\n\nIt expires in 10 minutes. If you didn't request this, ignore this email.`,
  };
}
...
function signupOtpTemplate({ name, otp }) {
  const safeName = (name || 'there').replace(/[<>]/g, '');
  return {
    subject: 'Your Connectik verification code',
    html: `
      <div style="font-family:Segoe UI,Arial,sans-serif;max-width:520px;margin:0 auto;padding:24px;color:#222">
        <h2 style="color:#0a66c2;margin:0 0 16px">Welcome to Connectik, ${safeName}!</h2>
        ...
        <p style="font-size:12px;color:#888;margin-top:32px">If you didn't sign up for Connectik, you can safely ignore this email.</p>
      </div>`,
    text: `Hi ${safeName},\n\nYour Connectik verification code is: ${otp}\n\nIt expires in 10 minutes. If you didn't sign up, ignore this email.`,
  };
}
